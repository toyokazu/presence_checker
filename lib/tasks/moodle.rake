...
namespace :moodle do
  ...
  task :merge_csv => :environment do
    begin
      ...
      dst_csv = ENV["MERGED_RECORDS_CSV"].nil? ? "#{Rails.root}/#{Batch::MERGED_RECORDS_CSV}" : "#{ENV["MERGED_RECORDS_CSV"]}"
...
#puts "csv_records: #{csv_records}"
      ...
      csv_moodles = csv_moodles.sort {|a,b| a[2].to_i <=> b[2].to_i}
...
#puts "moodle_attrs: #{moodle_attrs}"
      ...
      open(dst_csv, "wb") do |fout|
        fout.print "#{moodle_attrs[1]},#{moodle_attrs[2]},#{moodle_attrs.last}\r\n"
        #fout.print "#{moodle_attrs[0]},#{moodle_attrs[1]},#{moodle_attrs.last}\r\n"
